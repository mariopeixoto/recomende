...
import br.recomende.model.recommender.api.Mergeable;
...
public class RecommendUtil {
	...
	public <T extends Mergeable<T>> T invokeComponentMethod(Class<? extends Annotation> scannotation, Class<T> returnType, Object... params) throws IllegalArgumentException, IllegalAccessException, InvocationTargetException {
		...
		for(Object obj : searchers.values()) {
			...
			for (Method method : methods) {
				if (this.match(method, returnType, params)) {
					if (result == null) {
						result = (T) method.invoke(obj, params);
					} else {
						result.merge((T) method.invoke(obj, params));
					}
				}
			}
		}
	...
	private boolean match(Method method, Class<?> returnClass, Object...  params) {
		if (method != null) {
			...
			if (parametersTypes.length == params.length) {
				...
				return returnClass.isAssignableFrom(method.getReturnType());
			}
		}
		return false;
	}
